...
//if (!range && range.length < 2) throw new Error('Range must be an array with two numbers'); range?
...
let preferredTheme = window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches ? "night" : "day";
...
document.querySelector("[data-settings-form]").addEventListener("submit", (event) => {
    ...
    document.documentElement.style.setProperty('--color-dark', css[result.v].dark);
    document.documentElement.style.setProperty('--color-light', css[result.v].light);
    document.querySelector("[data-settings-overlay]").open = false;
});
...
document.querySelector('[data-search-form]').addEventListener('submit', (event) => {
    ...
    let filteredBooks = [...books]; // Create a copy of the original books array
    
    // Apply filters
    if (filters.genre !== 'any') {
        filteredBooks = filteredBooks.filter(book => book.genres.includes(filters.genre));
    }
